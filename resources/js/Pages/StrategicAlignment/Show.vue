...
// ตรวจจับการเปลี่ยนค่าฟิลเตอร์
watch(filterForm, debounce((value) => {
    router.get(
        route('strategic-alignments.show', props.modelData.id),
        value,
...
<template>
    <Head :title="`ยุทธศาสตร์: ${modelData.key}`" />
    <PeaSidebarLayout>
        <div class="py-8 px-6 max-w-[1600px] mx-auto space-y-6">
            <Link :href="route('strategic-alignments.index')" class="text-[#7A2F8F] font-bold hover:underline mb-2 inline-flex items-center gap-1">
                &larr; กลับไปหน้าจัดการยุทธศาสตร์
            </Link>

            <div class="bg-white p-6 rounded-xl shadow-sm border border-purple-200 border-l-8 border-l-[#4A148C] flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h2 class="text-2xl font-black text-[#4A148C]">🔗 ยุทธศาสตร์: {{ modelData.key }}</h2>
                    <p class="text-gray-600 mt-2 leading-relaxed">{{ modelData.description }}</p>
                </div>
                <div class="bg-purple-50 text-purple-800 font-bold px-4 py-2.5 rounded-lg border border-purple-100 whitespace-nowrap shrink-0">
            ...
            <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
                <div class="grid grid-cols-1 md:grid-cols-6 gap-3">
                    <div class="md:col-span-2 relative">
                        ...
                        <input v-model="filterForm.search" type="text" class="pl-10 w-full rounded-lg border-gray-300 focus:ring-[#7A2F8F] focus:border-[#7A2F8F] shadow-sm text-sm" placeholder="ค้นหาชื่องาน..." />
                    </div>

                    <div>
                        <select v-model="filterForm.type" class="w-full rounded-lg border-gray-300 text-sm focus:ring-[#7A2F8F] focus:border-[#7A2F8F] shadow-sm">
                    ...
                    <div>
                        <select v-model="filterForm.status" class="w-full rounded-lg border-gray-300 text-sm focus:ring-[#7A2F8F] focus:border-[#7A2F8F] shadow-sm">
                    ...
                    <div>
                        <select v-model="filterForm.division_id" class="w-full rounded-lg border-gray-300 text-sm focus:ring-[#7A2F8F] focus:border-[#7A2F8F] shadow-sm">
                    ...
                    <div class="flex gap-2">
                        <select v-model="filterForm.progress" class="flex-1 rounded-lg border-gray-300 text-sm focus:ring-[#7A2F8F] focus:border-[#7A2F8F] shadow-sm">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <table class="w-full text-left border-collapse">
                    <thead class="bg-gray-50 text-xs uppercase text-gray-500 border-b border-gray-200">
                        <tr>
                            ...
                            <th class="px-6 py-4 font-black w-32 text-center">ความคืบหน้า</th>
                    ...
                    <tbody class="divide-y divide-gray-100 text-sm">
                        ...
                        <tr v-for="item in workItems.data" :key="item.id" class="hover:bg-purple-50 transition group">
                            <td class="px-6 py-4">
                                <Link :href="route('work-items.show', item.id)" class="font-bold text-[#4A148C] group-hover:underline text-base line-clamp-2">
                                    {{ item.name }}
                                </Link>
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-2" :title="`ความคืบหน้า: ${item.progress || 0}%`">
                                    <div class="w-16 bg-gray-200 rounded-full h-2">
                                        <div class="h-2 rounded-full transition-all duration-500" :class="item.status === 'delayed' ? 'bg-red-500' : 'bg-[#7A2F8F]'" :style="{ width: (item.progress || 0) + '%' }"></div>
                                    </div>
                            ...
                            <td class="px-6 py-4 text-center">
                                <Link :href="route('work-items.show', item.id)" class="inline-flex items-center justify-center bg-white border border-gray-300 hover:border-[#7A2F8F] text-gray-600 hover:text-[#7A2F8F] p-2 rounded-lg shadow-sm transition" title="ดูรายละเอียดโครงการ">
                ...
                <div class="px-6 py-4 border-t border-gray-100 flex justify-between items-center bg-gray-50" v-if="workItems.links && workItems.links.length > 3">
                    <div class="flex flex-wrap gap-1">
                        <Link v-for="(link, key) in workItems.links" :key="key" :href="link.url || '#'" v-html="link.label" class="px-3 py-1.5 rounded-lg text-sm font-bold border shadow-sm transition" :class="link.active ? 'bg-[#7A2F8F] text-white border-[#7A2F8F]' : 'bg-white text-gray-600 border-gray-300 hover:bg-purple-50 hover:text-[#7A2F8F]'" preserve-scroll />
                    </div>
